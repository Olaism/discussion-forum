...
class LoginRequiredNewTopicTest(TestCase):
    def test_redirection(self):
        board = Board.objects.create(
            name="Django board", description="All about django"
        )
...
class NewTopicTests(TestCase):
    ...
    def test_new_topic_valid_post_data(self):
        data = {"subject": "A subject for you.", "message": "A message for everyone"}
    ...
    def test_new_post_invalid_data_with_empty_fields(self):
        data = {"subject": "My new subject", "messsage": "Another message"}
        response = self.client.post(self.url, data=data)
        self.assertEqual(self.response.status_code, 200)
